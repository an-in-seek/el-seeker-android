...
1. **기존 OAuth 계정 조회**: provider + providerUserId로 매칭 → 프로필 동기화 후 JWT 발급
2. **동일 이메일 회원 존재**: 기존 회원에 OAuth 계정 자동 연결 → JWT 발급
3. **신규 사용자**: 회원 + OAuth 계정 생성 → JWT 발급 (닉네임은 빈 문자열로 생성됨)

> 신규 가입 시 닉네임이 비어 있으므로, 앱에서 닉네임 설정 화면으로 유도하는 것을 권장합니다.
